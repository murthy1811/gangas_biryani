    ...
    if size:
        if item_id in list(cart.keys()):
            if size in cart[item_id]['items_by_size'].keys():
                ...
                messages.success(request, f'Updated size {size.upper()} {dish.name} quantity to {cart[item_id]["items_by_size"][size]}')
            else:
                cart[item_id]['items_by_size'][size] = quantity
        ...
        if item_id in list(cart.keys()):
            ...
            messages.success(request, f'Updated {dish.name} quantity to {cart[item_id]}')
        else:
            cart[item_id] = quantity
    ...
    if size:
        if quantity > 0:
            ...
            if not cart[item_id]['items_by_size']:
                cart.pop(item_id)
            messages.success(request, f'Removed size {size.upper()} {dish.name} from your bag')
    else:
        if quantity > 0:
            cart[item_id] = quantity
            messages.success(request, f'Updated {dish.name} quantity to {cart[item_id]}')
            ...
            cart.pop(item_id)
            messages.success(request, f'Removed {dish.name} from your bag')
